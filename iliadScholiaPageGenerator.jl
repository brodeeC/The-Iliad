...
# ╔═╡ fc0c39f2-4b84-11ef-25cf-dd0d091a6ff3
begin 
	using Markdown
	using PlutoUI
	using CitableBase
...
# ╔═╡ cd708cf1-5453-47e8-a88d-d53e4e490391
md""" # Generate HTML of all pages of the Iliad"""

# ╔═╡ e352c249-facc-4aae-8a71-c7381d54fdad
md""" Packages:"""

# ╔═╡ f769787e-7b99-4257-9027-6be5227844e8
md""" ## Necessary variables"""

# ╔═╡ 8f443d72-476a-40d0-b3ae-1e6a753a6c3e
src = hmt_cex()

# ╔═╡ e2e9859b-4ba4-4f2e-a7e0-33249066021d
normalizedtexts = hmt_normalized(src)

# ╔═╡ ef64a2ef-6b37-4e44-b289-f03d34dffd9b
textcatalog = hmt_textcatalog(src)

# ╔═╡ 80b91b16-58b0-4728-9a8a-070b4ef88153
textcatalog.entries

# ╔═╡ da689e94-a0de-4ddd-843f-f237fdafae9d
md""" ## HTML items"""

# ╔═╡ e7e6b36f-4c04-4064-a793-143ae3aac4a7
# Save a String to a file
begin

	function saveStrToFile(str ::String, filepath::String)
...
# ╔═╡ df31c945-6135-4122-91d5-b4478bf10888
md""" ## The head of the HTML page"""

# ╔═╡ bc8ff562-334d-45b1-9045-56516df38b35
htmlhead = """
				<!DOCTYPE html>
				<html lang="en">
				<head>
...
# ╔═╡ 57086b5a-eb3b-43a2-bd16-407cb97a8cb2
md""" ## Stylesheet of the HTML page"""

# ╔═╡ e2a292b1-ee63-428a-ab9c-d684a69ba8bc
htmlstyle = """
...
# ╔═╡ 3990276c-a72b-4916-9171-4254042ad95d
md""" ## Javascript Function

Function displays the content associated with a button

"""

# ╔═╡ 8d16df0f-5b93-456b-abba-6fc7c9556327
htmlscript = """<script>
...
# ╔═╡ 97d82793-5f35-49ec-9963-0908ab0d0407
md"""
...
# ╔═╡ f06abd3b-90b3-4f4e-bd8c-a5f79c9400d2
# Steps through a vector and creates a new vector of length size.
...
# ╔═╡ 35107e14-bcbf-4bc8-bab5-80d2092a9269
# Just MsA URNs
begin
	justMsA = filter(normalizedtexts.passages) do p
		msAUrn = CtsUrn("urn:cts:greekLit:tlg0012.tlg001.msA:")
		urncontains(msAUrn, p.urn)
	end
end

# ╔═╡ 9fa36046-d1f5-4dcf-8c17-fb0fdba24dd9
# Just URNS
begin

	justURNs = map(justMsA) do p
		p.urn
	end
	
end

# ╔═╡ a535eda4-b9ad-4054-95b3-c5c5cde88864
# no exemplar, and drop the line numbers
begin
	versionURNs = map(justURNs) do e
		tempU = dropexemplar(e)
		cpT = collapsePassageTo(tempU, 1)
	end
end

# ╔═╡ 59aa0e14-d2f3-4950-ae4d-37ce9acbf9e1
iliadBookURNs = unique(versionURNs)

# ╔═╡ 1abf2463-8cde-4688-ab4c-4a79f32de97b
#Get each book of the Iliad - normalized

function getBook(bookNumber::Int)
	filter(normalizedtexts.passages) do p
		myUrn = iliadBookURNs[bookNumber]
		urncontains(myUrn, p.urn)
	end
end

# ╔═╡ 71f6fc3f-cef8-4965-a336-8126c28b0619
# Get the passages of a given book
 
function getPassages(book::Vector{CitablePassage})
	map(book) do p
		p
	end
end

# ╔═╡ b2ad07d7-c3e0-4d83-8aa8-ef2d0ca5e906
scholia_index = hmt_commentary(src)[1]


# ╔═╡ 5b4f8358-3eaf-43f3-969a-608047b53a04
md""" ## Big Loop that puts all the functions and HTML together to create all the necessary pages"""

# ╔═╡ b28839df-dab9-438a-aaa3-42d62d3499ab
function urlFormation(currBook ::Int64, currPage ::Int64, lastpage)
	
	#Getting previous book info for URL formation
		if (currBook > 1)
			prevBook = currBook - 1
		else
			prevBook = 1
		end
		book2 = getBook(prevBook)
		passages2 = getPassages(book2)

		# The last page of the previous book
		lenPrevBook = length(sliding(passages2, 25, 25))

			prevPage = currPage - 1 # previous page
			nextPage = currPage + 1 # next page
			nextBook = currBook + 1 # next book
...
# ╔═╡ 161d4715-4500-4bbe-ab6e-d2fd4425ed9b
function titleFormation(currwindow, currPage, currBook)
	# Gets the first line
		if (currPage != 1) # If it's not the first page
		...
		# Last line number of the page
		lastnum = passageparts(lastline.urn)[2]

		# Top of the HTML, adds "tab" - title information
		return ("""<title>The Iliad: Book $currBook, Lines: $firstnum - $lastnum </title>
				<div class="header">The <em>Iliad </em>Book $currBook, Lines: $firstnum - $lastnum</div>""")
end

# ╔═╡ 8281de6c-3198-4596-b4a9-9c42bddac5b4
function getScholDetails(scholUrn)
	
	for i in 1 : length(textcatalog)
		entry = textcatalog.entries[i]
		entryUrn = entry.urn
		
		newUrn = dropexemplar(droppassage(scholUrn))
		if (urncontains(entry, newUrn))
			return entry.work
		end
	end
end

# ╔═╡ 22eb07d9-8ac8-42dd-9b9e-987b317f2e5e
function scholFormation(currUrn, currText, linenum)
	# String of scholia that belong to a line
	scholia = """\n<div class="scholia" id="scholia$linenum">"""

	# URNs and text of scholia at currUrn
	allSchol = scholia_on(currUrn, normalizedtexts, scholia_index)

	if (isempty(allSchol))
		return ""
	end
	
	count = 0 # Used to attach it to a specific button
	...
	### Generates HTML around all scholia lines ###
	schol = map(allSchol) do s
		if (length(s) < 2)
			lemma = ""
			comment = s[1].text
		else
		...
		count += 1 # Name each comment differently

		scholDetails = getScholDetails(s[1].urn)

		detStr = """<p><em>$scholDetails</em></p>"""

		if (isempty(arr1))
			push!(arr1, detStr)
		elseif(isempty(arr2) && !(detStr in scholArr))
			push!(arr2, detStr)
		elseif(isempty(arr3) && !(detStr in scholArr))
			push!(arr3, detStr)
		elseif(isempty(arr4) && !(detStr in scholArr))
			push!(arr4, detStr)
		elseif(isempty(arr5) && !(detStr in scholArr))
			push!(arr5, detStr)
		elseif(isempty(arr6) && !(detStr in scholArr))
			push!(arr6, detStr)
		elseif(isempty(arr7) && !(detStr in scholArr))
			push!(arr7, detStr)
		elseif(isempty(arr8) && !(detStr in scholArr))
			push!(arr8, detStr)
		end

		push!(scholArr, detStr)

		if (lemma != "")
			scholStr = ("""<p><span class = "lineNumber">&emsp;$lemma</span><button class="expand-button" onclick="toggleContent('comment$linenum.$count')">See Comment</button></p><div class="comment" id="comment$linenum.$count"><p>$comment</p></div>""")
		else
			scholStr = ("""<p><span class = "lineNumber">&emsp;No lemma entry</span><button class="expand-button" onclick="toggleContent('comment$linenum.$count')">See Comment</button></p><div class="comment" id="comment$linenum.$count"><p>$comment</p></div>""")
		end

		if (arr1[1] == detStr)
			push!(arr1, scholStr)
		elseif(arr2[1] == detStr)
			push!(arr2, scholStr)
		elseif(arr3[1] == detStr)
			push!(arr3, scholStr)
		elseif(arr4[1] == detStr)
			push!(arr4, scholStr)
		elseif(arr5[1] == detStr)
			push!(arr5, scholStr)
		elseif(arr6[1] == detStr)
			push!(arr6, scholStr)
		elseif(arr7[1] == detStr)
			push!(arr7, scholStr)
		else
			push!(arr8, scholStr)
		end
	...
	return scholia * str * "</div>" * "\n"
	
end

# ╔═╡ d050557a-268a-4a0d-b056-71664d5b1d6f
begin
	...
	for currBook in 4 : length(iliadBookURNs)
		...
		passages = getPassages(book)

		#Get only 25 lines
		window = sliding(passages, 25, 25)

		currPage = 1 # Current page number
		map(window) do p

			htmlfoot = urlFormation(currBook, currPage, length(window))

			# Gets the current page
			currwindow = window[currPage]

			htmltitle = titleFormation(currwindow, currPage, currBook)
			
			### Forms html around text along and finds Scholia ###
			lines = map(p) do line
				currUrn = dropexemplar(line.urn)
				currText = line.text
				linenum = passagecomponent(line.urn)

				# Creates one line
				if (linenum == "$currBook.title")
					linestr = """<p><span class = "title">$currText</span> </p>"""

				else
					scholia = scholFormation(currUrn, currText, linenum)
					
					if (!isempty(scholia))
						linestr = """<p><span class = "lineNumber">$linenum</span> &emsp;$currText<button class="expand-button" onclick="toggleContent('scholia$linenum')">See Scholia</button></p>""" * scholia * "\n"
					
					else
						linestr = """<p><span class = "lineNumber">$linenum</span> &emsp;$currText</p>""" * "\n"
					end
				end
			end

			#Join the html parts together and save it to the appropriate file
			myhtml = htmlhead * htmltitle * htmlstyle * join(lines, "\n") * htmlfoot * htmlscript
			fp = "/Users/brodee69/Documents/GitHub/The-Iliad/src/public/Book$currBook/page$currPage.html"
			saveStrToFile(myhtml, fp)
			currPage += 1
		end
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "83f4ce772e74db0506e5801253505af03db2d15e"
...
git-tree-sha1 = "d0e640fa4b6c5de04feee91da49a55074abb2f24"
